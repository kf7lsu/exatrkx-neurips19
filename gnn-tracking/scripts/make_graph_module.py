...
# consider only first MAX_NODES nodes
MAX_NODES = 100
input_graphs[0]['nodes'] = input_graphs[0]['nodes'][0:MAX_NODES]
input_graphs[0]['n_node'] = np.asarray(input_graphs[0]['nodes'].shape[0])
# cut edges that don't conncet first MAX_NODES nodes
mask = (input_graphs[0]['receivers'] < MAX_NODES) & (input_graphs[0]['senders'] < MAX_NODES)
input_graphs[0]['receivers'] = input_graphs[0]['receivers'][mask]
input_graphs[0]['senders'] = input_graphs[0]['senders'][mask]
